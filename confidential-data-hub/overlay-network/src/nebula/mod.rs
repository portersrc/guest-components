...
impl NebulaMesh {
    ...
    pub async fn init(&self, kbs_url: String) -> Result<()> {
        ...
        self.write_worker_config_to_file(kbs_url)?;
    ...
    async fn request_key_and_certs(&self, kbs_url: Ipv4Addr) -> Result<NebulaPluginResponse> {
        let mesh_ip = self.generate_mesh_ip(kbs_url)?;
    ...
    fn write_worker_config_to_file(&self, kbs_url: Ipv4Addr) -> Result<()> {
        let static_host_map_str = format!(
            "\"{}\": [\"{}:{}\"]",
            kbs_url, self.config.lighthouse_ip, LIGHTHOUSE_PORT
        );
    ...
    fn generate_mesh_ip(&self, kbs_url: Ipv4Addr) -> Result<Ipv4Addr> {
        ...
        let mesh_ip = (kbs_url & overlay_netmask) | (iface_ip & !overlay_netmask);
        Ok(mesh_ip)
    }
}
