...
impl NebulaMesh {
    ...
    pub async fn set_up(&self,
        ...
        info!("porter NebulaMesh::set_up");
        info!("porter String lighthouse_pub_ip: {}", lighthouse_pub_ip);
        info!("porter String pod_name: {}", pod_name);

        let is_lighthouse: bool = lighthouse_pub_ip.is_empty();
        let mut mesh_ip = LIGHTHOUSE_IP;
        if !is_lighthouse {
            info!("porter this is not a lighthouse.");
            mesh_ip = self.generate_mesh_ip()?;
        } else {
            info!("porter this is a lighthouse.");
        }
        ...
        info!("porter len of response from kbs: {:?}", response.len());
        info!("porter response from kbs: {:?}", response);
        info!("porter string response from kbs: {}", String::from_utf8_lossy(&response));
        ...
        info!("porter2 finished launching nebula");
        Ok(())
    }
    ...
    fn get_iface_of_default_gateway(&self) -> Result<String> {
        info!("porter read-from-proc-net-route called");
        ...
        for part in s {
            ...
            info!("porter iface: {}", iface);
            info!("porter destination: {}", destination);
            info!("porter mask: {}", mask);
            // FIXME ? Should mask also be checked?
            if destination == "00000000" {
                info!("porter break hit");
                break;
            }
        }
        info!("porter success for iface: {}", iface);
        Ok(iface.to_string())
    }
    ...
    fn generate_mesh_ip(&self) -> Result<Ipv4Addr> {
        ...
        info!("iface of the default gateway: {}", iface);
        info!("iface ip: {}", iface_ip);
        info!("iface mask: {}", iface_mask);
        info!("lighthouse ip: {}", LIGHTHOUSE_IP);
        info!("mesh ip: {}", mesh_ip);
        info!("Exiting generate_mesh_ip");
        Ok(mesh_ip)
    }
